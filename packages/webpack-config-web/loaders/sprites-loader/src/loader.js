...
const Spritesmith = require("leihuo-spritesmith");
...
module.exports = function (content) {
  ...
  Spritesmith.run(
    ...
    function handleResult(err, result) {
      ...
      const disableWebp = assets.find(
        (asset) => asset.indexOf("keep_origin") > -1,
      );
      if (disableWebp) {
        outputPath = path.join(outputPath, "keep_origin");
      }
      ...
      mkdirp(outputPath, function (err) {
        ...
        let spriteOutputPath = helper.getSpriteOutputPath(
          context,
          outputPath,
          options.name,
          result.image,
        ...
        for (let index = res.length - 1; index >= 0; index--) {
          ...
          let spriteCode = spriteCodeGenerator(
            spriteImage,
            eleWidth,
            eleHeight,
            needWidth,
          ...
          let currMatchContentAfter = content.substring(
              currMatchStart + currMatchLength,
            ),
          ...
          let spriteRequest = getSpriteRequest(
            context,
            resourcePath,
            spriteOutputPath,
          );
