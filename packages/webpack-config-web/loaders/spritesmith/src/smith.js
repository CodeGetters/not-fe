...
function Spritesmith(params) {
  ...
  if (typeof engineName === "string") {
    ...
    try {
      ...
      console.error(
        'Attempted to find spritesmith engine "' +
          engineName +
          '" but could not.',
      );
      console.error(
        'Please verify you have installed "' +
          engineName +
          '" and saved it to your `package.json`',
      );
      console.error("");
    ...
    try {
      ...
      /* eslint-disable no-console */
      console.error(
        'Attempted to load spritesmith engine "' +
          engineName +
          '" but could not.',
      );
      console.error(
        "Please verify you have installed its dependencies. Documentation should be available at ",
      );
      console.error("");
  ...
  if (!semver.satisfies(Engine.specVersion, SPEC_VERSION_RANGE)) {
    throw new Error(
      'Expected `engine` to have `specVersion` within "' +
        SPEC_VERSION_RANGE +
        '" ' +
        'but it was "' +
        Engine.specVersion +
...
Spritesmith.run = function (params, callback) {
  ...
  spritesmith.createImages(params.src, function handleImages(err, images) {
    ...
    spriteData.image.pipe(
      concat({ encoding: "buffer" }, function handleImage(buff) {
