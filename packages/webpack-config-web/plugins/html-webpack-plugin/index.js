...
class HtmlWebpackPlugin {
  ...
  apply(compiler) {
      ...
      compiler.hooks.compilation.tap(
        ...
        (compilation) => {
          ...
          const AsyncSeriesWaterfallHook =
            require("tapable").AsyncSeriesWaterfallHook;
          compilation.hooks.htmlWebpackPluginAlterChunks =
            new SyncWaterfallHook(["chunks", "objectWithPluginRef"]);
    ...
    (compiler.hooks
      ...
      const applyPluginsAsyncWaterfall =
        self.applyPluginsAsyncWaterfall(compilation);
  ...
  getMetaTags() {
    ...
    const metaTagAttributeObjects = Object.keys(this.options.meta).map(
      (metaName) => {
        const metaTagContent = this.options.meta[metaName];
        return typeof metaTagContent === "object"
          ? metaTagContent
          : {
              name: metaName,
  ...
  injectAssetsIntoHtml(html, assets, assetTags) {
    ...
    if (head.length) {
      ...
      if (!headRegExp.test(html)) {
        if (this.options.autoCreateHead === false) {
          html = head.join("") + html;
        } else {
  ...
  applyPluginsAsyncWaterfall(compilation) {
    ...
    return (eventName, requiresResult, pluginArgs) =>
      promisedApplyPluginsAsyncWaterfall(eventName, pluginArgs).then(
        (result) => {
